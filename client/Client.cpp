...
Client::Client(Config& config)
    : tcp_socket(config.ipaddr, config.port),
      udp_socket(config.ipaddr, config.port),
      game_state(config.unicode),
...
void Client::getNextCommand() {
  try {
    ...
    if (command_id == "exit") {
      exit = true;
      if (game_state.isGame()) quitHandler(game_state, udp_socket, command_stream);
      return;
    } else if (command_id == "?") {
      printHelp();
      return;
