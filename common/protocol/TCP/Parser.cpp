...
std::string TcpParser::parseFileName() { return parseVariableString(FNAME_MAX, ' '); }

/// @brief Confirms the next character is the argument delimeter (' ')
void TcpParser::next() { checkNextChar(' '); }

/// @brief Confirms the next character is the end packet character ('\n')
void TcpParser::end() { checkNextChar('\n'); }

/// @brief Parses the Packet ID
std::string TcpParser::parsePacketID() { return parseFixedString(PACKET_ID_LEN); }

/// @brief Parses a packet status (3 chars)
std::string TcpParser::parseStatus() { return parseFixedString(STATUS_CODE_LEN); }
...
std::string TcpParser::parsePlayerID() {
  ...
  try {
    ...
    if (n < 0 || n > PLID_MAX) throw InvalidPacketException();
