...
class SocketAddressResolveError : public CommonError {
 ...
 public:
  SocketAddressResolveError(int err)
      : CommonError(std::string(errorMsg) + gai_strerror(err)) {};
};
...
class SocketAlreadyCreatedError : public CommonError {
 ...
 public:
  SocketAlreadyCreatedError()
      : CommonError(std::string(errorMsg) + std::strerror(errno)) {};
};
